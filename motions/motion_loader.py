...
class MotionLoader:
    ...
    def __init__(
        ...
        if len(set(self._dof_names)) != len(self._dof_names):
            raise ValueError(f"Motion contains duplicate joint names: {self._dof_names}")
        if expected_dof_names is not None and set(self._dof_names) != set(expected_dof_names):
            missing = sorted(set(expected_dof_names) - set(self._dof_names))
            unexpected = sorted(set(self._dof_names) - set(expected_dof_names))
            raise ValueError(f"Motion joint schema does not match JOSE G1: missing={missing}, unexpected={unexpected}")
    ...
    def get_dof_index(self, dof_names: list[str]) -> list[int]:
        ...
        if len(set(dof_names)) != len(dof_names):
            raise ValueError(f"Duplicate DOF names requested: {dof_names}")
        indexes = []
    ...
    def get_body_index(self, body_names: list[str]) -> list[int]:
        ...
        if len(set(body_names)) != len(body_names):
            raise ValueError(f"Duplicate body names requested: {body_names}")
        indexes = []
